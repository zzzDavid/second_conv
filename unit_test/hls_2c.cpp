...
int main(){
    int32_t v2[1][4][8][8];
    top_2c(only_1_2c,w2_ran,v2);
    ofstream myfile;
    myfile.open ("compare_2c.py", std::ios::app);
    //myfile << "import numpy as np \n";
    myfile << "hls_cpp_only1_w_rand= np.array([";
    for (int i=0; i<4;i++){
        for(int j=0; j<8; j++){
            for(int m=0;m<8;m++){
                myfile << v2[0][i][j][m]<<",";
...
void top_2c(
  ...
  bool conv2_pad[1][2][10][10]; // L480
  l_cc: for (int cc = 0; cc < 2; cc++) {        // L480
    l_hh: for (int hh = 0; hh < 10; hh++) {     // L480
      l_ww: for (int ww = 0; ww < 10; ww++) {   // L480
        ...
        bool v19 = v0[0][cc][(hh - 1)][(ww - 1)];       // L15
        bool v20 = 0;   // L480
